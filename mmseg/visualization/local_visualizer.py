...
class SegLocalVisualizer(Visualizer):
    ...
    def set_image_(self, image: np.ndarray) -> None:
        ...
        # prompt = np.asarray([range(500), range(500)])
        # self.ax_save.scatter(prompt[:, 0], prompt[:, 1], c='white', s=1000, marker='x')
        self.ax_save.imshow(
            image,
    ...
    def _draw_sem_seg(self,
        ...
        for label, color in zip(labels, colors):
            ...
            # print(mask[sem_seg[0] == label, :].shape, color)

        # mask = np.zeros_like(image, dtype=np.uint8)
        if prompt is not None:  # [H, W, 3]
            prompt = prompt[:, :, 0]
            # print(prompt.shape, mask.shape)

            # prompt = [[20, 60, 70], [50, 60, 70]]
            # self.ax_save.scatter(
            #     prompt[:, 0], prompt[:, 1], c=colors, s=20, marker='x')
            prompt_position = (prompt == 1.).int().numpy()
            # print(mask[prompt_position, :].shape)
            # mask[prompt == 1., :] = [255, 0, 0]

            # for i in range(prompt.shape[0]):
            #     for j in range(prompt.shape[1]):
            #         if prompt[i, j] == 1:
            #             mask = cv2.circle(mask, (i, j), 15, (255, 255, 255), 4)
            # mask = cv2.circle(mask, ((20, 50), (60, 70)), 15, (255, 255, 255), 4)  # 4是thickness，厚度
        ...
        if prompt is not None:
            # print((prompt == 1).sum(), color_seg[prompt == 1, :].shape)
            # print(prompt.shape, color_seg.shape)
            # color_seg[prompt != 1, :] = (255, 255, 255)
            color_seg[prompt == 0, :] = (255, 255, 255)
            color_seg[prompt == 1, :] = (0, 0, 0)
            # color_seg = cv2.circle(color_seg, ((500, 500)), 15, (255, 255, 255), 4)  # 4是thickness，厚度

        self.set_image_(color_seg)
        return color_seg
    ...
    def add_datasample(
        ...
        if out_file is not None:
            ...
            # 不知道为啥这里要手动保存，可能local visualize哪里没读取成功
