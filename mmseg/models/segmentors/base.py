...
class MyBaseModel(BaseModel):
    ...
    def test_step(self, data: Union[dict, tuple, list],
        ...
        # return self._run_forward(data, mode=mode)  # type: ignore

        if isinstance(data, dict):
            results = self(**data, mode=mode, prompt=prompt, **kwargs)
...
class BaseSegmentor(MyBaseModel, metaclass=ABCMeta):
    ...
    # 数据处理改不了了，如果后面要修改的话就在forward这里看看怎么处理。这里是一度比
    def forward(self,
                inputs: Tensor,
                data_samples: OptSampleList = None,
                mode: str = 'tensor',
                prompt=None, **kwargs) -> ForwardResults:
        ...
        if prompt is not None:
            # 这里的input已经做了归一化了，所以我把prompt修改在这里
            if prompt.shape != inputs.shape:
                prompt = resize(prompt, size=inputs.shape[-2:], mode='bilinear',
                                align_corners=False, warning=False)
            inputs = inputs + prompt.expand(inputs.shape[0], -1, -1, -1)
        if mode == 'loss':
            return self.loss(inputs, data_samples)
        elif mode == 'predict':
            return self.predict(inputs, data_samples, **kwargs)  # 这里可能需要给更多参数
        elif mode == 'tensor':
            return self._forward(inputs, data_samples)  # inputs和data_samples在base_model得到
        elif mode == 'forward_fea':
            return self._forward_prompt(inputs, data_samples)
        else:
