...
class Extractor(nn.Module):
    def __init__(self, dim, num_heads=6, n_points=4, n_levels=1, deform_ratio=1.0,
        ...
        # ratio=deform_ratio
        # 这里想修改成attn，感觉有点难度，因为原来的shape不匹配,导致无法计算
...
class InteractionBlockPrompt(nn.Module):
    ...
    def forward(self, x, c1, c2, c3, stage, hw_shape, backbone='mit', vgg_layers=None):
        H, W = hw_shape
        bs, channel, dim = x.shape
        deform_inputs1, deform_inputs2 = self.deform_inputs(hw_shape, x.device) 

        c = torch.cat([c1, c2, c3], dim=1)  # b*86016*dim  c1c2c3类似与prompt的multiscale
        ...
        if backbone == 'swin':
            ...
            for i, blk in enumerate(stage):
                # x_injector = blk(x_injector, H, W)  # daformer是旧版的mit用这个
                x_injector = blk(x_injector, (H, W))  # 新版用这个
